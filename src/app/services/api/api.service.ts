...
export class ApiService {
    ...
    getCharacters(
        ...
        return forkJoin(
            pagesArray.map(page => this.getPageOfCharacters(page))
        ).pipe(map(pages => [].concat.apply([], pages)));
    }

    getPageOfCharacters(page: number): Observable<Character[]> {
        return this.http
            .get<SWAPI>(`${this.swapiUrl}/people/?page=${page}`)
            .pipe(map(swapi => swapi.results as Character[]));
    }
