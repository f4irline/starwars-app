...
export class ChartService {
    ...
    charactersToGenderByEyeColor(characters: Character[]): MultiSeriesData[] {
        ...
        characters.forEach(char => {
            const gender =
                char.gender === 'n/a'
                    ? 'None'
                    : this.formatService.capitalizeFirstCharacter(char.gender);
            const eyeColor = this.formatService.capitalizeFirstCharacter(
                char.eye_color
            );
            ...
            const existingGender = eyeColorsWithValues
                .get(gender)
                .find(color => color.name === eyeColor);

            if (existingGender) {
                existingGender.value += 1;
                existingGender.extra.characters = [
                    ...existingGender.extra.characters,
                    char,
                ];
            } else {
        ...
        const gendersByEyeColor: MultiSeriesData[] = Array.from(
            eyeColorsWithValues
        ).map(color => ({
            name: color[0],
            series: color[1],
    ...
    charactersToBmiData(characters: Character[]): SeriesData[] {
        const bmiWithClasses = new Map<
            BmiClass,
            { val: number; characters: Character[] }
        >();
        ...
        characters.forEach(char => {
            ...
            const bmi =
                !isNaN(mass) && !isNaN(height)
                    ? this.mathService.calculateBmi(
                          parseFloat(char.mass),
                          parseFloat(char.height)
                      )
                    : undefined;
            ...
            bmiWithClasses.has(obesityClass)
                ? bmiWithClasses.set(obesityClass, {
                      val: bmiWithClasses.get(obesityClass).val + 1,
                      characters: [
                          ...bmiWithClasses.get(obesityClass).characters,
                          char,
                      ],
                  })
                : bmiWithClasses.set(obesityClass, {
        ...
        const bmiClasses: SeriesData[] = Array.from(bmiWithClasses)
            .map(bmi => ({
                name: bmi[0],
                value: bmi[1].val,
                extra: {
    ...
    private sortBmiData(a: SeriesData, b: SeriesData): number {
        return (
            this.sortedBmiClasses.indexOf(a.name as BmiClass) -
            this.sortedBmiClasses.indexOf(b.name as BmiClass)
        );
    }
}
